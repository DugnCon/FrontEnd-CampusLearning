...
const EditCourse = () => {
  ...
  // Initialize modules as an empty array to prevent reduce errors
  ...
  const tabItems = [
    ...
    },
    {
      ...
      label: (
        <span>
          Xem trước {' '}
          {Array.isArray(modules) && modules.reduce((count, module) => count + (module.lessons ? module.lessons.filter(l => l.IsPreview).length : 0), 0) > 0 && (
            <Badge count={Array.isArray(modules) ? modules.reduce((count, module) => count + (module.lessons ? module.lessons.filter(l => l.IsPreview).length : 0), 0) : 0} />
          )}
        </span>
      ),
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        // Fetch course modules and ensure it's an array
        ...
        // Set modules to empty array on error to prevent reduce errors
        setModules([]);
