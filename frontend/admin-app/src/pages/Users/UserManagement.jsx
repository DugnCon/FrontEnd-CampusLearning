...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Map BE keys to FE keys
      const mappedUsers = userData.map(user => ({
        UserID: user.userID,
        Username: user.username || 'Không xác định',
        Email: user.email || 'Không có email',
        FullName: user.fullName || user.username || 'Không xác định',
        Role: user.role || 'STUDENT',
        AccountStatus: user.accountStatus || 'ACTIVE',
        CreatedAt: user.createdAt || new Date().toISOString(),
        LastLoginAt: user.lastLoginAt || null,
        School: user.school || 'Không có',
      ...
      const activeUsers = mappedUsers.filter(user => user.AccountStatus === 'ACTIVE').length;
      const lockedUsers = mappedUsers.filter(user => user.AccountStatus === 'LOCKED').length;
      const adminUsers = mappedUsers.filter(user => user.Role === 'ADMIN').length;
      ...
      // Mock data with new BE key structure
      const mockUsers = [
        {
          UserID: 1,
          Username: 'admin',
      ...
      setStats({
        ...
        locked: mockUsers.filter(user => user.AccountStatus === 'LOCKED').length,
