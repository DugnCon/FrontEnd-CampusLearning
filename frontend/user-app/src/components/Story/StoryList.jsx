...
const StoryList = ({ orientation = 'horizontal', showTimeline = false, onStoryEnd, onViewStory }) => {
    ...
    const getFullMediaUrl = (mediaUrl) => {
        ...
        console.log('🔍 StoryList - Original mediaUrl:', mediaUrl);
        ...
        if (mediaUrl.startsWith('/uploads/')) {
            ...
            console.log('🔄 StoryList - Using domain URL:', fullUrl);
            return fullUrl;
        }
    ...
    const fetchStories = async () => {
        try {
            ...
            console.log('📚 StoryList - Stories data:', data);
    ...
    // 🔥 SỬA: DÙNG getFullMediaUrl CHO RENDER STORY ITEM
    const renderStoryItem = (story) => {
        const fullMediaUrl = getFullMediaUrl(story.mediaUrl);
        const fullUserImageUrl = getFullMediaUrl(story.user?.image);
                ...
                <div className="relative w-full h-40 rounded-lg overflow-hidden bg-gray-200">
                    {story.mediaType === 'image' ? (
                        <img
                            ...
                            onError={(e) => {
                                console.error('❌ StoryList - Image failed:', fullMediaUrl);
                            }}
                        />
                        ...
                        <video
                            ...
                            crossOrigin="anonymous"
                            onError={(e) => {
                                console.error('❌ StoryList - Video failed:', fullMediaUrl);
                            }}
                        >
