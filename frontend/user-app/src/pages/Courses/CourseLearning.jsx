...
const CourseLearning = () => {
  ...
  const requestVideoSummary = async () => {
    ...
    try {
      const videoTitle = currentLesson.title || 'Bài học video';
      const courseTitle = course.title || 'Khóa học';
      const description = currentLesson.content || currentLesson.description || 'Không có mô tả chi tiết';

      const prompt = `Tóm tắt nội dung video sau bằng tiếng Việt, định dạng Markdown đẹp mắt:
  ...
  Yêu cầu:
  - Viết ngắn gọn, dễ hiểu, chỉ tập trung kiến thức chính
  - Dùng tiếng Việt tự nhiên
  - Bắt buộc dùng Markdown: # ## ###, danh sách -, in đậm **, bảng nếu cần
  - Không thêm lời chào, giải thích, chỉ trả về đúng nội dung tóm tắt

  Bắt đầu luôn bằng tiêu đề # Tóm tắt video`;
      ...
      const response = await axios.post(
        `https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
        {
          ...
          // Quan trọng: tắt hết safety để không bị block (với key free vẫn ok)
          safetySettings: [
      ...
      // PHẦN QUAN TRỌNG NHẤT – GỘP TẤT CẢ PARTS LẠI
      const candidates = response.data.candidates;
      if (!candidates || candidates.length === 0) {
        const blockReason = response.data.promptFeedback?.safetyRatings
          ? 'Bị chặn do nội dung nhạy cảm'
          : 'Không có phản hồi từ Gemini';
        throw new Error(blockReason);
      }

      // Gộp hết tất cả text từ mọi part (có thể 1 hoặc 10 phần cũng lấy hết)
      ...
      toast.success('Tóm tắt video thành công!');

    } catch (error) {
      console.error('Lỗi Gemini:', error.response?.data || error.message);

      if (error.response?.status === 429) {
        toast.error('Hết lượt miễn phí rồi bro! Đợi 1-2 phút hoặc tạo key mới');
      } else if (error.message.includes('BLOCK')) {
        toast.error('Nội dung bị Gemini chặn (hiếm), thử đổi tiêu đề video');
      } else {
        toast.error('Không tạo được tóm tắt: ' + (error.message || 'Lỗi mạng'));
      }
    } finally {
